---
sidebar_position: 8
---

# Step 8 - Receive and Process Authentication Response

## Overview

After an authentication request is processed, the Sub-AUA receives a two-layered response over HTTPS:

1. **Outer AUA Response (`<xml>`)**: Transport/wrapper layer containing `client_id`, `res_code`, `res_hash`, `res_data`, and `res_msg`.
2. **Inner Authentication XML (`<AuthRes>`)**: The actual authentication payload, Base64-encoded inside the `<res_data>` tag.

To determine final authentication success, the Sub-AUA must parse the outer response, decode `<res_data>`, and evaluate the `<AuthRes>` XML.

---

## Response Processing Workflow

```text
Receive HTTP Response (200 OK)
            │
            ▼
 Parse Outer AUA XML (<xml>)
            │
            ▼
 Verify client_id & res_code
            │
            ▼
 Base64 Decode <res_data>
            │
            ▼
 Parse Inner XML (<AuthRes>)
            │
            ▼
  Verify XML Signature
            │
            ▼
 Check 'ret' Attribute
            │
      ┌─────┴─────┐
      ▼           ▼
   ret="y"     ret="n"
      │           │
      ▼           ▼
   SUCCESS     FAILURE
```

---

## 1. Outer AUA Response Structure

The AUA returns an outer XML wrapper.

```xml
<?xml version="1.0" encoding="utf-8"?>
<xml>
    <client_id>AUA-CHP</client_id>
    <res_code>10</res_code>
    <res_hash>BASE64_ENCODED_RESPONSE_HASH</res_hash>
    <res_data>PD94bWwgdmVyc2lvbj0iMS4wIiBlbmNvZGluZz0iVVRGLTgi...</res_data>
    <res_msg>Success from CSC AAG.</res_msg>
</xml>
```

**OR**

```xml
<?xml version="1.0" encoding="utf-8"?> 
<xml>
	<client_id>AUA-CHP</client_id>
	<res_code>10</res_code>
	<res_hash>UbHZv3KToV6b8xjg/jHXdzXhajbNDKKKD2ZlHMOk9xbio/ZZAN8q3jYIY2ayg2uL1PgbkHWZF0QW1i4nCq58ysD6n6XblFFgybSP/dgSOoM=</res_hash>
	<res_data></res_data>
	<res_msg>Success from CSC AAG.</res_msg>
</xml>
```

### Outer Response Fields

| Field | Description |
| :--- | :--- |
| `client_id` | Sub-AUA client identifier. Verify this matches expected configuration. |
| `res_code` | Gateway status code returned by the AUA (e.g., `10`). |
| `res_hash` | Hash associated with the response data for integrity verification. |
| `res_data` | **Base64-encoded** Authentication Response XML (`<AuthRes>`). |
| `res_msg` | Descriptive status message from the AUA. |

---

## 2. Inner Authentication Response (`<AuthRes>`)

Base64 decoding `<res_data>` reveals the inner authentication response payload:

```xml
<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<AuthRes
    code="51047f1e0ff5494bacd35a17b66d0de9"
    info="04{...}"
    ret="y"
    ts="2024-05-02T11:42:39.187+05:30"
    txn="2405021142340169818591290pmg">

    <Signature xmlns="http://www.w3.org/2000/09/xmldsig#">
        ...
    </Signature>
</AuthRes>
```

### Key `<AuthRes>` Attributes

| Attribute | Description | Action Required |
| :--- | :--- | :--- |
| `ret` | **Final authentication result** (`y` or `n`). | **Primary flag for success or failure.** |
| `txn` | Authentication Transaction ID. | Must match the original request `txn` for correlation. |
| `code` | Response or error code. | Store for auditing/tracking. |
| `ts` | Response timestamp. | Validate for auditing. |
| `info` | Additional metadata (device, status, etc.). | Treat as sensitive; do not display to end users. |

---

## 3. Critical Verification Rules

> ⚠️ **HTTP 200 ≠ Authentication Success**
> - **`HTTP 200`** only indicates that the API request reached the server.
> - **`<res_code> 10`** only indicates success at the AUA gateway wrapper level.
> - **`<AuthRes ret="y">`** is the **only** definitive indicator of a successful authentication.

### Result Handling

* **Success (`ret="y"`)**: Continue business logic, grant requested service, log transaction ID (`txn`).
* **Failure (`ret="n"`)**: Read error/response codes, display appropriate user message, allow retry if permitted.
* **Signature Verification**: Validate the XML `<Signature>` inside `<AuthRes>` to confirm data integrity and origin.

---

## Validation Checklist & Best Practices

- [ ] **Validate HTTP Status:** Confirm HTTP `200`.
- [ ] **Parse Outer XML:** Verify `client_id`, `res_code`, and `res_hash`.
- [ ] **Decode Payload:** Base64 decode `<res_data>` to reveal `<AuthRes>`.
- [ ] **Verify Signature:** Validate the digital signature on `<AuthRes>`.
- [ ] **Correlate Transaction:** Confirm response `txn` matches request `txn`.
- [ ] **Check Authentication Result:** Evaluate `ret="y"` vs `ret="n"`.
- [ ] **Security & Privacy:**
  - Never log raw PID XML, sensitive biometric data, session keys, or unencrypted `info`.
  - Retain only minimum transaction metadata needed for audit and compliance.